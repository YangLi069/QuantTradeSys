...
class Excave_Indic_Base:
    ...
    def plot_Aver_Cross(self, stock_df):
        ...
        list_signal = list_signal.dropna(axis=0, how='any')

        dispCont_List = [
            "M20&M60 金叉:\n" + "日期:" + list_signal.index[x].strftime('%Y-%m-%d') + '\n' if list_signal[x] > 0
            else "M20&M60 死叉:\n" + "日期:" + list_signal.index[x].strftime('%Y-%m-%d') + '\n'
            for x in range(0, len(list_signal.index))]
    ...
    def plot_Jump_Thrd(self, stock_df):
        # 计算涨跌幅（今收-昨收）/昨收*100%
        # 判断向上跳空缺口和向下跳空缺口
        stock_df['changeRatio'] = stock_df.Close.pct_change() * 100
        # 增加昨收序列
        stock_df['preClose'] = stock_df.Close.shift(1)
        # 跳空阈值
        jump_threshold = stock_df.Close.median() * 0.01
        jump_pd = pd.DataFrame()
        for kl_index in np.arange(0, stock_df.shape[0]):
            today = stock_df.loc[kl_index]
            # 检测跳空缺口
        ...
        jump_pd = jump_pd[(np.abs(jump_pd.changeRatio) > 2) & (jump_pd.Volume > 20000000)]
        dispCont_List = ["向上跳空:\n" + "日期:" + jump_pd.index[x].strftime('%Y-%m-%d') +
                         '\n' + "缺口值:" + str('%.2f' % jump_pd.jump_power[x]) +
                         '\n' if jump_pd.jump_power[x] > 0
                         else "向下跳空:\n" + "日期:" + jump_pd.index[x].strftime('%Y-%m-%d') +
                             '\n' + "缺口值:" + str('%.2f' % jump_pd.jump_power[x]) +
                              '\n' for x in range(0, len(jump_pd.index))]

        print(jump_pd.filter(['jump_power', 'preClose', 'changeRatio', 'Close', 'Volumn']))
...
class FactorBuyAverBreak:
    ...
    def fit_day(self, kl_index, today, stock_df):
        ...
        if today.Close > stock_df.Close[day_ind - self.xd + 1:day_ind + 1].mean():
            print('FactorBuyAverBreak for info', kl_index, today.Close,
                  stock_df.Close[day_ind - self.xd + 1:day_ind + 1].mean())
...
class FactorSellAverBreak:
    ...
    def fit_day(self, kl_index, today, stock_df):
        ...
        if today.Close < stock_df.Close[day_ind - self.xd + 1:day_ind + 1].mean():
            print('FactorSellAverBreak for info', kl_index, today.Close,
                  stock_df.Close[day_ind - self.xd + 1:day_ind + 1].mean())
...
class FactorBuyNdayBreak:
    ...
    def fit_day(self, kl_index, today, stock_df):
        ...
        if today.Close == stock_df.Close[day_ind - self.xd + 1:day_ind + 1].max():
            print('FactorBuyNdayBreak for info', kl_index, today.Close,
                  stock_df.Close[day_ind - self.xd + 1:day_ind + 1].max())
            return self.make_buy_order()
...
class FactorSellNdayBreak:
    ...
    def fit_day(self, kl_index, today, stock_df):
        ...
        if today.Close == stock_df.Close[day_ind - self.xd + 1:day_ind + 1].min():
            print('FactorSellNdayBreak for info', kl_index, today.Close,
                  stock_df.Close[day_ind - self.xd + 1:day_ind + 1].min())
            return self.fit_sell_order()
...
class QuantPickTimeSys:
    def __init__(self, kl_pd):
        ...
        # buy_factors: 买入因子序列，序列中的对象为dict，每一个dict针对一个具体因子
        # sell_factors: 卖出因子序列，序列中的对象为dict，每一个dict针对一个具体因子
        # 初始化买入因子列表
    ...
    def init_buy_factors(self, buy_factors):
        ...
        for factor_class in buy_factors:
            ...
            if not isinstance(factor, FactorBuyAverBreak) \
                    and not isinstance(factor, FactorBuyNdayBreak):  # 判断factor为基于FactorBuyBreak实例
    ...
    def init_sell_factors(self, sell_factors):
        ...
        for factor_class in sell_factors:
            ...
            if not isinstance(factor, FactorSellAverBreak) \
                    and not isinstance(factor, FactorSellNdayBreak):  # 判断factor为基于FactorBuyBreak实例
    ...
    def _day_task(self, kl_index, today):
        ...
        fact_buy = 1 if (buy_buf == (cnt + 1)) else 0
            ...
            if signal > 0:  # 买入
                ...
                if is_win == True:  # 避免未买先卖
                    ...
                    if self.kl_pd.Close[end] < self.kl_pd.Close[start]:  # 赔钱显示绿色
                        subplotP0.fill_between(self.kl_pd.index[start:end], 0,
                                               self.kl_pd.Close[start:end], color='green', alpha=0.38)

                    else:  # 赚钱显示红色
                        subplotP0.fill_between(self.kl_pd.index[start:end], 0,
                                               self.kl_pd.Close[start:end], color='red', alpha=0.38)
